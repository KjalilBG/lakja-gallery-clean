  ...
  return (
    <label className="flex items-start gap-4 rounded-[24px] border border-slate-200 bg-slate-50 px-5 py-5 transition hover:border-slate-300 hover:bg-white">
      ...
      <span className="block space-y-1">
        <span className="block text-sm font-black uppercase tracking-[0.14em] text-slate-900">{title}</span>
  ...
  return (
    <div className="space-y-6">
      ...
      <form action={saveSiteSettingsAction} className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div className="space-y-6">
          <section className="rounded-[30px] border border-slate-200 bg-white px-4 py-4 shadow-[0_14px_35px_rgba(15,23,42,0.05)]">
              ...
              {tabs.map((tab) => (
                <a
                  ...
                  className={`inline-flex items-center justify-center rounded-full px-4 py-2.5 text-xs font-extrabold uppercase tracking-[0.14em] transition md:text-sm ${
                    activeTab === tab.key
                      ? "bg-slate-950 text-white shadow-[0_10px_25px_rgba(15,23,42,0.14)]"
              ...
              <section className="rounded-[30px] border border-slate-200 bg-white px-6 py-5 shadow-[0_14px_35px_rgba(15,23,42,0.05)]">
                <div className="grid gap-3 md:grid-cols-3">
                  <div className="rounded-[22px] border border-slate-100 bg-slate-50 px-4 py-4">
                    <p className="text-[10px] font-extrabold uppercase tracking-[0.22em] text-slate-400">Estado publico</p>
                    <p className="mt-2 text-lg font-black tracking-tight text-slate-950">
                      {settings.maintenanceMode ? "Mantenimiento" : "Activo"}
                    </p>
                  </div>
                  <div className="rounded-[22px] border border-slate-100 bg-slate-50 px-4 py-4">
                    <p className="text-[10px] font-extrabold uppercase tracking-[0.22em] text-slate-400">Galerias destacadas</p>
                    <p className="mt-2 text-lg font-black tracking-tight text-slate-950">{settings.featuredAlbumIds.length}</p>
                  </div>
                  <div className="rounded-[22px] border border-slate-100 bg-slate-50 px-4 py-4">
                    <p className="text-[10px] font-extrabold uppercase tracking-[0.22em] text-slate-400">Funciones activas</p>
                    <p className="mt-2 text-lg font-black tracking-tight text-slate-950">{enabledFeatureCount}/4</p>
                  </div>
                </div>
              </section>
              ...
              >
                <div className="grid gap-5">
                  <div className="space-y-2">
                    ...
                    <input name="shareTitle" defaultValue={settings.shareTitle} maxLength={90} className="w-full rounded-[22px] border border-slate-200 bg-slate-50 px-5 py-4 text-base font-semibold text-slate-900 outline-none transition focus:border-lime-300 focus:bg-white focus:ring-4 focus:ring-lime-100" />
                  </div>
                  <div className="space-y-2">
                    <FieldLabel>Descripcion al compartir</FieldLabel>
                    <textarea name="shareDescription" defaultValue={settings.shareDescription} rows={4} maxLength={180} className="w-full rounded-[22px] border border-slate-200 bg-slate-50 px-5 py-4 text-base font-medium leading-7 text-slate-700 outline-none transition focus:border-lime-300 focus:bg-white focus:ring-4 focus:ring-lime-100" />
                  </div>
                  <div className="space-y-2">
                ...
                <SectionCard eyebrow="Redes" title="Canales sociales" description="Actualiza las ligas globales que usa la home y el popup de descarga.">
                  <div className="grid gap-5">
                    <div className="space-y-2">
                      ...
                      <input name="instagramUrl" defaultValue={settings.instagramUrl} className="w-full rounded-[22px] border border-slate-200 bg-slate-50 px-5 py-4 text-base font-medium text-slate-700 outline-none transition focus:border-lime-300 focus:bg-white focus:ring-4 focus:ring-lime-100" />
                    </div>
                    <div className="space-y-2">
                      <FieldLabel>Facebook</FieldLabel>
                      <input name="facebookUrl" defaultValue={settings.facebookUrl} className="w-full rounded-[22px] border border-slate-200 bg-slate-50 px-5 py-4 text-base font-medium text-slate-700 outline-none transition focus:border-lime-300 focus:bg-white focus:ring-4 focus:ring-lime-100" />
                    </div>
                  </div>
                </SectionCard>
                <SectionCard eyebrow="Contacto" title="WhatsApp" description="Controla el número general y el mensaje inicial del botón flotante.">
                  <div className="grid gap-5">
                    <div className="space-y-2">
                      ...
                      <input name="whatsappNumber" defaultValue={settings.whatsappNumber} className="w-full rounded-[22px] border border-slate-200 bg-slate-50 px-5 py-4 text-base font-medium text-slate-700 outline-none transition focus:border-lime-300 focus:bg-white focus:ring-4 focus:ring-lime-100" />
                    </div>
                    <div className="space-y-2">
                      <FieldLabel>Mensaje inicial</FieldLabel>
                      <textarea name="whatsappMessage" defaultValue={settings.whatsappMessage} rows={4} className="w-full rounded-[22px] border border-slate-200 bg-slate-50 px-5 py-4 text-base font-medium leading-7 text-slate-700 outline-none transition focus:border-lime-300 focus:bg-white focus:ring-4 focus:ring-lime-100" />
                    </div>
                  </div>
                </SectionCard>
              </div>
          ...
          <section className="rounded-[30px] border border-slate-200 bg-white px-6 py-5 shadow-[0_14px_35px_rgba(15,23,42,0.05)]">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <p className="text-sm leading-6 text-slate-500">Los cambios aplican al sitio apenas se guarda la configuracion.</p>
            </div>
          </section>
        </div>
        ...
        <aside className="space-y-6 xl:sticky xl:top-28 xl:self-start">
          ...
          <section className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-[0_14px_35px_rgba(15,23,42,0.05)]">
            <p className="text-xs font-extrabold uppercase tracking-[0.24em] text-slate-400">Tab activa</p>
            <div className="mt-4 rounded-[22px] border border-slate-100 bg-slate-50 px-4 py-4">
              <p className="text-lg font-black tracking-tight text-slate-950">{tabs.find((tab) => tab.key === activeTab)?.label}</p>
              <p className="mt-2 text-sm leading-6 text-slate-500">Trabaja por bloques para que el panel se sienta mas limpio y facil de operar.</p>
            </div>
          </section>
