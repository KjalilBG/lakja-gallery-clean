import { CheckCircle2, NotebookPen, ShieldCheck, Sparkles, Wrench } from "lucide-react";

import { SiteAdminNotesPanel } from "@/components/admin/site-admin-notes-panel";
import { SiteBrandTab } from "@/components/admin/site-brand-tab";
import { getAdminNotebookByEmail } from "@/lib/admin-notes";
...
type SiteTab = "general" | "home" | "experience" | "maintenance" | "notes";
...
function getActiveTab(input?: string): SiteTab {
  ...
  const allowedTabs: SiteTab[] = ["general", "home", "experience", "maintenance", "notes"];
  return allowedTabs.includes(input as SiteTab) ? (input as SiteTab) : "general";
}
...
export default async function SiteSettingsPage({ searchParams }: SiteSettingsPageProps) {
  ...
  const [settings, adminAlbums, adminNotebook, resolvedSearchParams] = await Promise.all([
    getSiteSettings(),
    getAdminAlbums(),
    getAdminNotebookByEmail(currentUserEmail),
  ...
  const superAdminEmails = (process.env.SUPER_ADMIN_EMAILS ?? "")
    ...
    .map((value) => value.trim())
    .filter(Boolean);
  const tabs: Array<{ key: SiteTab; label: string; hint: string }> = [
    { key: "general", label: "General", hint: "Marca, compartir y contacto" },
    { key: "home", label: "Home", hint: "Hero y galerias destacadas" },
    { key: "experience", label: "Experiencia", hint: "WhatsApp, descargas y favoritas" },
    { key: "maintenance", label: "Mantenimiento", hint: "Estado publico del sitio" },
    { key: "notes", label: "Notas", hint: "Bloc personal del admin" }
  ];
  ...
  const currentTabMeta = tabs.find((tab) => tab.key === activeTab) ?? tabs[0];
    ...
    <div className="space-y-6">
      <section className="overflow-hidden rounded-[34px] border border-slate-200 bg-[linear-gradient(135deg,#ffffff,#f8fafc)] p-8 shadow-[0_14px_35px_rgba(15,23,42,0.06)] dark:border-slate-700 dark:bg-[linear-gradient(135deg,#111827,#0f172a)] dark:shadow-[0_18px_40px_rgba(2,6,23,0.42)]">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-3 text-5xl font-black tracking-tight text-slate-900 dark:text-white">Sitio</h1>
            <p className="mt-4 text-base leading-7 text-slate-500 dark:text-slate-300">
              Un panel mas claro para ajustar la experiencia publica, ordenar la home y dejarte notas privadas de seguimiento.
            </p>
          </div>
      ...
      {saved ? (
        <div className="flex items-center gap-3 rounded-[26px] border border-lime-200 bg-lime-50 px-5 py-4 text-sm font-semibold text-lime-800 dark:border-lime-500/30 dark:bg-lime-500/12 dark:text-lime-200">
          ...
          Los cambios de esta seccion se guardaron correctamente.
        </div>
      ) : null}
      ...
      {activeTab === "notes" ? (
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
          <SiteAdminNotesPanel
            initialNotes={adminNotebook.content}
            initialUpdatedAt={adminNotebook.updatedAt ? adminNotebook.updatedAt.toISOString() : null}
          />
          ...
          <aside className="space-y-6 xl:sticky xl:top-28 xl:self-start">
            <section className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-[0_14px_35px_rgba(15,23,42,0.05)] dark:border-slate-700 dark:bg-slate-900">
              <p className="text-xs font-extrabold uppercase tracking-[0.24em] text-slate-400 dark:text-slate-500">Seccion actual</p>
              <div className="mt-4 rounded-[22px] border border-slate-100 bg-slate-50 px-4 py-4 dark:border-slate-800 dark:bg-slate-800/70">
                <div className="flex items-center gap-3">
                  <div className="rounded-full bg-amber-100 p-2.5 text-amber-700 dark:bg-amber-500/12 dark:text-amber-300">
                    <NotebookPen className="size-4" />
                  </div>
                  <div>
                    <p className="text-lg font-black tracking-tight text-slate-950 dark:text-white">{currentTabMeta.label}</p>
                    <p className="mt-1 text-sm leading-6 text-slate-500 dark:text-slate-300">{currentTabMeta.hint}</p>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="rounded-[30px] border border-slate-200 bg-[linear-gradient(135deg,#fafaf9,#ffffff)] p-6 shadow-[0_14px_35px_rgba(15,23,42,0.05)] dark:border-slate-700 dark:bg-[linear-gradient(135deg,#111827,#0f172a)]">
              <p className="text-xs font-extrabold uppercase tracking-[0.24em] text-slate-400 dark:text-slate-500">Como usarlo</p>
              <ul className="mt-4 space-y-3 text-sm leading-6 text-slate-600 dark:text-slate-300">
                <li>Anota pendientes visuales, copy, bugs o ideas para futuras mejoras.</li>
                <li>Se guarda solo mientras escribes, sin boton manual.</li>
                <li>Se sincroniza con tu cuenta de super admin para verlo en otros dispositivos.</li>
              </ul>
            </section>
          </aside>
        </div>
      ) : (
        <form action={saveSiteSettingsAction} className={activeTab === "general" ? "space-y-6" : "grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]"}>
          ...
          <div className="space-y-6">
                  ...
                  <div className="grid gap-3 md:grid-cols-3">
                    <div className="rounded-[22px] border border-slate-100 bg-slate-50 px-4 py-4 dark:border-slate-800 dark:bg-slate-800/70">
                      ...
                      <p className="mt-2 text-lg font-black tracking-tight text-slate-950 dark:text-white">
                        {settings.maintenanceMode ? "En mantenimiento" : "Activo"}
                      </p>
                    </div>
                    <div className="rounded-[22px] border border-slate-100 bg-slate-50 px-4 py-4 dark:border-slate-800 dark:bg-slate-800/70">
            ...
            {activeTab === "home" ? (
              <>
                <SectionCard eyebrow="Home" title="Textos principales" description="Ajusta la bienvenida, el tono de la portada y el mensaje central sin tocar código.">
                ...
                <SectionCard eyebrow="Curaduria" title="Galerias destacadas" description="Elige manualmente que albumes publicados quieres ver primero en la home. Si desmarcas todos, se limpia la seleccion destacada.">
                  <input type="hidden" name="featuredAlbumIdsIntent" value="present" />
            ...
            {activeTab === "experience" ? (
              <>
                ...
                <SectionCard eyebrow="Funciones" title="Controles para cliente" description="Ajusta rapido lo que el cliente ve y puede hacer en la web publica.">
            ...
            {activeTab === "maintenance" ? (
              <>
                <section className="rounded-[30px] border border-slate-200 bg-white px-6 py-5 shadow-[0_14px_35px_rgba(15,23,42,0.05)] dark:border-slate-700 dark:bg-slate-900">
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="rounded-[22px] border border-slate-100 bg-slate-50 px-4 py-4 dark:border-slate-800 dark:bg-slate-800/70">
                      <p className="text-[10px] font-extrabold uppercase tracking-[0.22em] text-slate-400 dark:text-slate-500">Estado actual</p>
                      <p className="mt-2 text-lg font-black tracking-tight text-slate-950 dark:text-white">
                        {settings.maintenanceMode ? "Mantenimiento activo" : "Sitio visible"}
                      </p>
                    </div>
                    <div className="rounded-[22px] border border-slate-100 bg-slate-50 px-4 py-4 dark:border-slate-800 dark:bg-slate-800/70">
                      <p className="text-[10px] font-extrabold uppercase tracking-[0.22em] text-slate-400 dark:text-slate-500">Impacto</p>
                      <p className="mt-2 text-sm font-semibold leading-6 text-slate-700 dark:text-slate-200">
                        Afecta la home y las galerias publicas. Al desactivarlo, el sitio vuelve a mostrarse normalmente.
                      </p>
                    </div>
                  </div>
                </section>

                <SectionCard eyebrow="Mantenimiento" title="Control publico" description="Activa o desactiva la pausa temporal del sitio con un mensaje claro para tus clientes.">
            ...
            <section className="rounded-[30px] border border-slate-200 bg-white px-6 py-5 shadow-[0_14px_35px_rgba(15,23,42,0.05)] dark:border-slate-700 dark:bg-slate-900">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <p className="text-sm leading-6 text-slate-500 dark:text-slate-300">Te mantenemos en esta misma seccion despues de guardar para que el flujo sea mas claro.</p>
              </div>
            </section>
          </div>
          ...
          {activeTab !== "general" ? (
            <aside className="space-y-6 xl:sticky xl:top-28 xl:self-start">
              <section className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-[0_14px_35px_rgba(15,23,42,0.05)] dark:border-slate-700 dark:bg-slate-900">
                ...
                <div className="mt-4 rounded-[22px] border border-slate-100 bg-slate-50 px-4 py-4 dark:border-slate-800 dark:bg-slate-800/70">
                  <p className="text-lg font-black tracking-tight text-slate-950 dark:text-white">{currentTabMeta.label}</p>
                  <p className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-300">{currentTabMeta.hint}</p>
                </div>
              </section>
                ...
                <div className="mt-4 space-y-4">
                  <div className="rounded-[22px] border border-slate-100 bg-slate-50 px-4 py-4">
                    <div className="flex items-center gap-2 text-slate-900">
                      ...
                      <p className="text-sm font-black uppercase tracking-[0.14em]">Home</p>
                    </div>
                  ...
                  <div className="rounded-[22px] border border-slate-100 bg-slate-50 px-4 py-4">
                    <div className="flex items-center gap-2 text-slate-900">
                      ...
                      <p className="text-sm font-black uppercase tracking-[0.14em]">Estado publico</p>
                    </div>
                    <p className="mt-2 text-sm font-semibold text-slate-700">
                      {settings.maintenanceMode ? "Modo mantenimiento activo" : "Sitio operando normalmente"}
                    </p>
                  </div>
                </div>
              </section>
